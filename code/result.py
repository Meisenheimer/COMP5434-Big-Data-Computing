...
if __name__ == "__main__":
    ...
    for folder in folder_list:
        ...
        if (model.lower() == "logistic"):
            ...
            print(f"{model},\t{alpha_1}\t{alpha_2}\t{degree}\t{text[1].split(',')[1]}\t{text[2].strip()}\t{text[3].strip()}")
            ...
            print(f"{model},\t{dropout}\t{hidden_size}\t{text[1].split(',')[1]}\t{text[2].strip()}\t{text[3].strip()}")
            ...
            print(f"{model},\t{max_depth}\t{n_threshold}\t{criterion}\t{text[1].split(',')[1]}\t{text[2].strip()}\t{text[3].strip()}")
        # exit(0)

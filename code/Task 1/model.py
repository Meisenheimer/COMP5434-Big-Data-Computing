...
class DecisionTree():
    ...
    def _find_best_split(self, x, y):
        ...
        for feature in range(x.shape[1]):
            ...
            if len(thresholds) == 0:  # 如果没有可用的阈值
                continue

            gain = self._calculate_information_gain(x, y, feature)
            if len(gain) == 0:  # 如果没有有效的增益值
                continue
    ...
    def _calculate_information_gain(self, x, y, feature):
        thresholds = torch.unique(x[:, feature])
        if len(thresholds) == 0:  # 如果没有可用的阈值，返回全零增益
            return torch.zeros(1, device=self.device, dtype=DTYPE)

        parent_impurity = self._calculate_impurity(y.unsqueeze(0))  # (1, n_samples) → (1,)
        gain = torch.zeros(len(thresholds), device=self.device, dtype=DTYPE)
        ...
        for i, threshold in enumerate(thresholds):
            ...
            if n_left == 0 or n_right == 0:  # 如果划分后某一边没有样本，跳过
                gain[i] = 0.0
                continue

            # 计算左、右子节点的纯度
            left_impurity = self._calculate_impurity(y[mask].unsqueeze(0))
            right_impurity = self._calculate_impurity(y[~mask].unsqueeze(0))

            # 信息增益 = 父节点纯度 - (左子节点占比 * 左纯度 + 右子节点占比 * 右纯度)
            gain[i] = parent_impurity - (n_left / len(y)) * left_impurity - (n_right / len(y)) * right_impurity
    ...
    def _make_leaf_node(self, y):
        ...
        if len(counts) == 0:  # 处理空样本的情况...
            return {"class": 0}  # 默认返回label 0
...
class RandomForest():
    ...
    def fit(self, x, y):
        for i in tqdm(range(self.n_estimator)):
            index_data = random.sample(range(x.shape[0]), round(x.shape[0]))
